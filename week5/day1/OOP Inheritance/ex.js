...
class Principal extends Person {
    ...
    expelStudent(Student) {
        let Estudent = this.students.findIndex(s => s.name == Student.name)
        this.students.splice(Estudent, 1)
    }
    transferStudent(Student, Principal) {
       let Istudent =  this.students.findIndex(s => s.name == Student.name)
       let relevantStudent = this.students[Istudent]
       this.students.splice(Istudent, 1)
       Principal.students.push(relevantStudent)
